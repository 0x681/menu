...
do
    -- fill=false so sections auto-size to their content height
...
do
    local L = TabWorld:column({})
    local R = TabWorld:column({})

    local s = L:section({ name = "Lighting" })
    bind(s:addToggle({ name = "No Fog",            flag = "w_nofog",  default = State.flags.no_fog            }), "no_fog")
    bind(s:addToggle({ name = "Bright Lighting",   flag = "w_bright", default = State.flags.bright_lighting   }), "bright_lighting")
    ...
    task.spawn(function()
        while library.gui and library.gui.Parent do
            if pl_list and pl_list.refresh_options then
                pcall(function() pl_list.refresh_options(refresh_playerlist()) end)
            end
            task.wait(3)
        end
    end)
